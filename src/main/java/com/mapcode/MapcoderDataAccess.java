...
public class MapcoderDataAccess {

    private static byte[] fileData;

    private MapcoderDataAccess() {
    }

    static {
        // *** read whole file into memory ***
        try {
            InputStream inputStream = MapcoderDataAccess.class
                    .getResourceAsStream("/mminfo.dat");
    ...
    private static int _ubyte(int i) {
        int u = fileData[i];
        if (u < 0) {
            u += 256;
        }
    ...
    public static int data_flags(int i) {
        return _ubyte(i * 20 + 16) + _ubyte(i * 20 + 17) * 256;
    }

    public static int _long(int i) {
        return _ubyte(i) + (_ubyte(i + 1) << 8) + (_ubyte(i + 2) << 16) + (_ubyte(i + 3) << 24);
    }

    public static int smartdiv(int i) {
        return _ubyte(i * 20 + 18) + _ubyte(i * 20 + 19) * 256;
    }

    private final static int[] data_start = {

    0, 3, 6, 9, 13, 16, 18, 19, 30, 31, 33, 35, 37, 42, 44, 47, 51, 54, 56, 58, 60, 62, 64, 68, 71, 79, 81, 116, 121,
            131, 133, 135, 138, 143, 155, 159, 165, 167, 169, 173, 176, 183, 185, 187, 189, 193, 195, 202, 208, 212,
