...
public final class Mapcode {
     ...
     * @return Non-empty, ordered list of Mapcode information records, see {@link com.mapcode.MapcodeInfo}.
    ...
    public static List<MapcodeInfo> encode(
        ...
        @Nonnull final List<MapcodeInfo> results = Encoder.encode(latDeg, lonDeg, null, false, false, true);
        assert results != null;
     ...
     *                            be null.
     * @return List of Mapcode information records, see {@link com.mapcode.MapcodeInfo}. This list is empty if no
    ...
    public static List<MapcodeInfo> encode(
        ...
        // Call Mapcode encoder.
        @Nonnull final List<MapcodeInfo> results =
     ...
     * @param latDeg Latitude, accepted range: -90..90.
     * @param lonDeg Longitude, accepted range: -180..180.
     * @return Shortest Mapcode (always exists), see {@link com.mapcode.MapcodeInfo}.
     * @throws IllegalArgumentException Thrown if latitude or longitude are out of range.
     */
    @Nonnull
    public static MapcodeInfo encodeToShortest(
        final double latDeg,
        final double lonDeg) throws IllegalArgumentException {
        checkRange("latDeg", latDeg, Point.LAT_DEG_MIN, Point.LAT_DEG_MAX);
        checkRange("lonDeg", lonDeg, Point.LON_DEG_MIN, Point.LON_DEG_MAX);
        ...
        // Call Mapcode encoder.
        @Nonnull final List<MapcodeInfo> results = Encoder.encode(latDeg, lonDeg, null, false, true, true);
        assert results != null;
        assert results.size() == 1;
     ...
     * @param restrictToTerritory Try to encode only within this territory, see {@link com.mapcode.Territory}. Cannot
     *                            be null.
     * @return Shortest Mapcode, see {@link com.mapcode.MapcodeInfo}.
     * @throws IllegalArgumentException Thrown if latitude or longitude are out of range.
     ...
     * @param latDeg Latitude, accepted range: -90..90.
     * @param lonDeg Longitude, accepted range: -180..180.
     * @return International unambiguous Mapcode (always exists), see {@link com.mapcode.MapcodeInfo}.
     * @throws IllegalArgumentException Thrown if latitude or longitude are out of range.
     */
    @Nonnull
    public static MapcodeInfo encodeToInternational(
        final double latDeg,
        final double lonDeg) throws IllegalArgumentException {
        checkRange("latDeg", latDeg, Point.LAT_DEG_MIN, Point.LAT_DEG_MAX);
        checkRange("lonDeg", lonDeg, Point.LON_DEG_MIN, Point.LON_DEG_MAX);
        ...
        // Call Mapcode encoder.
        @Nonnull final List<MapcodeInfo> results = encode(latDeg, lonDeg);
        assert results != null;
        assert results.size() >= 1;
